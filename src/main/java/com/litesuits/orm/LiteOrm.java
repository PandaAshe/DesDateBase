...
public abstract class LiteOrm extends SQLiteClosable implements DataBase {
    ...
    public SQLiteDatabase openOrCreateDatabase() {
        ...
        mTableManager = new TableManager(mConfig.dbName, mHelper.getReadableDatabase(Config.PW));
        //TODO  mHelper.getWritableDatabase();
        return mHelper.getWritableDatabase(Config.PW);       //TODO
    }
            ...
            if (mTableManager.isSQLMapTableCreated(table1.name, table2.name)) {
                CollSpliter.split(key1List, SQLStatement.IN_TOP_LIMIT, new CollSpliter.Spliter<String>() {
                    ...
                    public int oneSplit(ArrayList<String> list) throws Exception {
                        ...
                        Querier.doQuery(mHelper.getReadableDatabase(Config.PW), stmt, new Querier.CursorParser() {
    ...
    @Override
    public boolean execute(net.sqlcipher.database.SQLiteDatabase db, SQLStatement statement) {
        acquireReference();
        try {
            if (statement != null) {
    ...
    public boolean dropTable(String tableName) {
        acquireReference();
        try {
            return SQLBuilder.buildDropTable(tableName).execute(mHelper.getWritableDatabase(Config.PW));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            releaseReference();
    ...
    public long queryCount(QueryBuilder qb) {
        ...
        try {
            if (mTableManager.isSQLTableCreated(qb.getTableName())) {
                SQLiteDatabase db = mHelper.getReadableDatabase(Config.PW);
    ...
    public int update(WhereBuilder where, ColumnsValue cvs, ConflictAlgorithm conflictAlgorithm) {
        acquireReference();
        try {
            SQLiteDatabase db = mHelper.getWritableDatabase(Config.PW);
    ...
    public synchronized SQLiteDatabase getReadableDatabase() {
        return mHelper.getReadableDatabase(Config.PW);
    }

    @Override
    public synchronized SQLiteDatabase getWritableDatabase() {
        return mHelper.getWritableDatabase(Config.PW);
    }
    ...
    @Override
    public SQLiteDatabase openOrCreateDatabase(String path, net.sqlcipher.database.SQLiteDatabase.CursorFactory factory) {
        path = mConfig.context.getDatabasePath(mConfig.dbName).getPath();
        return SQLiteDatabase.openOrCreateDatabase(path,Config.PW,factory);
    }

    @Override
    public boolean deleteDatabase() {
        String path = mHelper.getWritableDatabase(Config.PW).getPath();
        justRelease();
    ...
    protected void justRelease() {
        if (mHelper != null) {
            mHelper.getWritableDatabase(Config.PW).close();
            mHelper.close();
            mHelper = null;
        }
